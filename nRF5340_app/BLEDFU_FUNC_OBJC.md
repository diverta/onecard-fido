...
|7|反映待機|DFUイメージ転送完了後、Zephyrアプリケーションが自動起動し、<br>USB HID接続が検出されるまで待機します。|
|8|バージョン情報照会|BLE接続が再開されたら、BLE経由でバージョン情報照会を実行します。|
...
DFUの処理において受け渡しされるデータ（DFUトランザクションデータ）につきましては、別ドキュメント「[BLE DFU機能のトランザクション](../nRF5340_app/BLEDFU_TRANSACTION.md)」をご参照願います。

#### 参考プログラム

このプログラムを制作するにあたり、参考にしたプログラムの詳細につきましては、別ドキュメント「[BLE DFU機能のサンプルコード調査](../nRF5340_app/BLEDFU_FUNC_IOS.md)」をご参照願います。
