...
// PIN更新処理
//
static uint16_t update_pin(uint8_t *pin_buf)
{
    // PIN／リトライカウンターをセットで登録
    // （PINのリトライカウンターは、デフォルトに設定）
    if (ccid_piv_object_pin_set(TAG_PIV_PIN, pin_buf, PIN_DEFAULT_RETRY_CNT) == false) {
...
// リトライカウンター更新処理
//
static uint16_t update_pin_retries(uint8_t retries)
{
    // 登録されているPIN／リトライカウンターを取得
    // （登録されていない場合はデフォルトが戻ります）
    if (ccid_piv_object_pin_get(TAG_PIV_PIN, work_buf, NULL) == false) {
        return SW_UNABLE_TO_PROCESS;
    }

    // PIN／リトライカウンターをセットで登録
    if (ccid_piv_object_pin_set(TAG_PIV_PIN, work_buf, retries) == false) {
        return SW_UNABLE_TO_PROCESS;
    }
