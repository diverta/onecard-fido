...
static char *TAG9C_TEMP = "408982D155434D1BC96302206CA038733CD1E9AC33C2D1AACF0B73E8E384153C";
static char *TAG9D_TEMP = "0A1D83DB42913860951CE5A87084328BB1168F002F8763BCD6A0B7E318AD627D";
...
{
#if CCID_PIV_OBJECT_TEST
    *size = convert_hexstring_to_bytes(TAG9A_TEMP, buffer);
    return true;
#else
    // 後日正式に実装予定です。
...
{
#if CCID_PIV_OBJECT_TEST
    *size = convert_hexstring_to_bytes(TAG9C_TEMP, buffer);
    return true;
#else
    // 後日正式に実装予定です。
...
{
#if CCID_PIV_OBJECT_TEST
    *size = convert_hexstring_to_bytes(TAG9D_TEMP, buffer);
    return true;
#else
    // 後日正式に実装予定です。
