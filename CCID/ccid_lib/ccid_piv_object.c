...
static char *TAG05_KEY_TEMP = "84EF66C1D3B9A5061331A27879D0B1D9E33817FF5BBC1C26969627E6FE3C43FE";
...
{
#if CCID_PIV_OBJECT_TEST
    *size = convert_hexstring_to_bytes(TAG05_KEY_TEMP, buffer);
    return true;
#else
    // 後日正式に実装予定です。
    fido_log_debug("Keypair for PIV Authentication is requested (%d bytes)", *size);
    return false;
#endif
}
