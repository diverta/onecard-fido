...
uint8_t *fido_flash_cert_data(void)
{
    ...
    uint32_t *cert_buffer = skey_cert_data + 1;
...
uint32_t fido_flash_cert_data_length(void)
{
    // 証明書データ格納領域の長さを取得
    // (証明書データ格納領域の１ワード目)
    uint32_t cert_buffer_length = *skey_cert_data;
...
bool fido_flash_skey_cert_data_prepare(uint8_t *data, uint16_t length)
{
    ...
    // (エンディアンは変換せずにそのまま格納)
    skey_cert_data[0] = (uint32_t)cert_length;
    memcpy(skey_cert_data + 1, data + 32, cert_length);
    return true;
}
