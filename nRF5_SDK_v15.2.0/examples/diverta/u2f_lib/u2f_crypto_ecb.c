...
#define NRF_LOG_MODULE_NAME u2f_crypto_ecb
#include "nrf_log.h"
...
void u2f_crypto_ecb_generate_keyhandle(uint8_t *p_appid_hash, uint8_t *private_key_value, uint32_t private_key_length)
{
    ...
    u2f_crypto_ecb_encrypt(keyhandle_base_buffer, data_length, keyhandle_buffer);
}
...
void u2f_crypto_ecb_restore_keyhandle_base(uint8_t *keyhandle_value, uint32_t keyhandle_length)
{
    ...
    memset(keyhandle_base_buffer, 0, sizeof(keyhandle_base_buffer));
    uint16_t data_length = 64;
    u2f_crypto_ecb_decrypt(keyhandle_buffer, data_length, keyhandle_base_buffer);
 }
