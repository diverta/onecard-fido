...
namespace ToolAppCommon
{
    ...
    internal class CCIDDevice
    {
        ...
        private bool EstablishContext()
        {
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                if (ret == CCIDDeviceConst.SCARD_E_NO_SERVICE) {
                    AppLogUtil.OutputLogError("SCardEstablishContext returns SCARD_E_NO_SERVICE");
                } else {
                    AppLogUtil.OutputLogError(string.Format("SCardEstablishContext returns {0}", ret));
                }
                return false;
            }
            return true;
        ...
        private bool ListReaders()
        {
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                // 検出失敗
                AppLogUtil.OutputLogError("CCIDデバイスを確認できません。");
                return false;
            }
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                // 検出失敗
                AppLogUtil.OutputLogError("CCIDデバイス名称を取得できませんでした。");
                return false;
            }
            ...
            AppLogUtil.OutputLogDebug(string.Format("CCIDデバイス[{0}]を検出しました。", ReaderName));
            return true;
        }
        ...
        private bool Connect()
        {
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                AppLogUtil.OutputLogError(string.Format("CCIDデバイスに接続できませんでした。RC={0}", ret));
                return false;
            }
            AppLogUtil.OutputLogDebug(string.Format("CCIDデバイス[{0}]に接続しました。", ReaderName));
            return true;
        }
        ...
        public bool Transmit(byte[] sendBuffer)
        {
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                AppLogUtil.OutputLogError(string.Format("CCIDデバイスへの送信処理が失敗しました。RC={0}", ret));
                RecvLength = 0;
                return false;
            }
        ...
        public void Disconnect()
        {
            ...
            if (ret != CCIDDeviceConst.SCARD_S_SUCCESS) {
                AppLogUtil.OutputLogError(string.Format("CCIDデバイス切断処理が失敗しました。RC={0}", ret));
            } else {
                AppLogUtil.OutputLogDebug(string.Format("CCIDデバイス[{0}]から切断しました。", ReaderName));
            }
